...
const khetAIAgentFlow = ai.defineFlow(
  ...
  async (input) => {
    // Prepare the payload based on whether an image is provided
    ...
    try {
      ...
      console.error('Error with prompt call:', error);
      output = null;
    }

    // Check if 'output' is valid
    if (!output || !output.content || !Array.isArray(output.content.parts)) {
      console.error('Invalid output received:', output);
      return {
        response: "I'm sorry, I couldn't find an answer to your question. Please try rephrasing it.",
        audio: "",  // Provide an empty response for the audio
      };
    }

    // Map the response parts to extract the relevant text
    const responseText = output.content.parts
      .map((p: { text?: string; toolResponse?: { name: string, output: any } }) => {
        if (p.text) return p.text;
        if (p.toolResponse) {
          // Extract relevant summary/diagnosis from the tool's output
